...
class Processor(object):
    ...
    def __init__(self, dataset, model, optimizer, start_epoch, batch_size):
        ...
        if torch.cuda.is_available():
            ...
            with open("./save_ecdt/out.txt", "a") as f:  # 打开文件以便写入
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch+1, self.__dataset.num_epoch):
            ...
            print('\nVal result: Slot f1 score: {:.6f}, Intent accuracy score: {:.6f}, '
            ...
            with open("./save_ecdt/out.txt", "a") as f:
                print('\nVal result: Slot f1 score: {:.6f}, Intent accuracy score: {:.6f}, '
                      'Semantic accuracy score: {:.6f}.'.format(dev_f1_score, dev_acc, dev_sent_acc), file=f)
                print('Test result: Slot f1 score: {:.6f}, Intent accuracy score: {:.6f}, '
                      'Semantic accuracy score: {:.6f}.'.format(test_f1_score, test_acc, test_sent_acc), file=f)
            ...
            if dev_metric > best_dev_metric:
                ...
                model_save_dir = os.path.join(self.__dataset.save_dir, "dev_model")
                ...
                print('\n -------------------------------- Update for new best Val results --------------------------')
                print('\n[Epoch {:2d}]: Best Val result: Slot f1 score : {:.6f}, Intent accuracy : {:.6f}, '
                      'Semantic acc : {:.6f}.'.format(epoch, dev_f1_score, dev_acc, dev_sent_acc))
                print('                 Test result: Slot f1 score: {:.6f},  Intent accuracy : {:.6f}, '
                      'Semantic acc : {:.6f}.'.format(test_f1_score, test_acc, test_sent_acc))
                print('\n---------------------------------- Val Save Successfully ----------------------------------')
                with open("./save_ecdt/out.txt", "a") as f:
                    print(
                        '\n -------------------------------- Update for new best Val results -------------------------',
                        file=f)
                    print('\n[Epoch {:2d}]: Best Val result: Slot f1 score : {:.6f}, Intent accuracy : {:.6f}, '
                          'Semantic acc : {:.6f}.'.format(epoch, dev_f1_score, dev_acc, dev_sent_acc), file=f)
                    print('                 Test result: Slot f1 score: {:.6f},  Intent accuracy : {:.6f}, '
                          'Semantic acc : {:.6f}.'.format(test_f1_score, test_acc, test_sent_acc), file=f)
                    print(
                        '\n---------------------------------- Val Save Successfully ----------------------------------',
                        file=f)
            ...
            if test_metric > best_test_metric:
                ...
                # test_f1, test_acc, test_sent_acc = test_f1, test_acc, test_sent_acc 
                ...
                torch.save(self.__model, os.path.join(model_save_dir, "model.pkl"))
                torch.save(self.__dataset, os.path.join(model_save_dir, 'dataset.pkl'))
                # torch.save_ecdt(self.__optimizer, os.path.join(model_save_dir, 'optimizer.pkl'))
                state = {'model': self.__model, 'dataset': self.__dataset, 'optimizer': self.__optimizer,
                         'epoch': epoch}
                torch.save(state, os.path.join(model_save_dir, "test_model_epoch.pkl"))

                print(
                    '\n ********************************** Update for new best Test results **********************************')
                print('\n                  Val result: Slot f1 score : {:.6f}, Intent accuracy : {:.6f}, '
                      'Semantic acc : {:.6f}.'.format(dev_f1_score, dev_acc, dev_sent_acc))
                print('[Epoch {:2d}]: Best Test result: Slot f1 score : {:.6f}, Intent accuracy : {:.6f}, '
                      'Semantic acc : {:.6f}.'.format(epoch, test_f1_score, test_acc, test_sent_acc))
                ...
                with open("./save_ecdt/out.txt", "a") as f:
                    print(
                        ...
                        file=f)
                    print('\n                  Val result: Slot f1 score : {:.6f}, Intent accuracy : {:.6f}, '
                          'Semantic acc : {:.6f}.'.format(dev_f1_score, dev_acc, dev_sent_acc), file=f)
                    print('[Epoch {:2d}]: Best Test result: Slot f1 score : {:.6f}, Intent accuracy : {:.6f}, '
                          'Semantic acc : {:.6f}.'.format(epoch, test_f1_score, test_acc, test_sent_acc), file=f)
                    print(
                        '\n********************************** Test Save Successfully **********************************',
                        file=f)
